...
Изразите в С++ се оценяват. Последния семинар видяхме, че изрази от типа (x < y) се оценява до булева стойност.
...
int main()
{
    ...
    x = (5 + 7); // Пример 1: Израза (5 + 7) връща стойност 12, което е и новата стойност на х
...
* Изразът в пример 2 ще се оцени до променлива (ще се върне променливата y). Стойността на променливата y ще послужи за нова стойност на х.
...
int main()
{
    ...
    (x = 7) = 9; // Синтактично валидно. Стойността на х сега е 9, израза в скобите връща х.

    (5 + 7) = 9; // Грешка - Няма как да дадем стойност на стойност.
}
```
...
Оператора за инкрементиране (++) е унарен оператор (приема един аргумент, за разлика от бинарните, приемащи два), който увеличава стойността на променлива с единица. Еквивалентно имаме оператор за декрементиране (--), който намаля стойността на променлива с единица. Синтаксисът може да бъде:
...
```
* Префиксния оператор инкрементира променливата и връща нейната **нова** стойност.
* Постфиксния оператор инкрементира променливата и връща нейната **стара** стойност.

Всъщност, постфиксния оператор връща **стойност** а префиксния **променлива**. Как можем да проверим това нагледно?
...
```
Как разбираме какъв би бил резултата при оценка на израза?

Можем да сметнем (120 / 40) / 10 = 0, но можем и да сметнем 120 / (40 / 10) = 30.

**Всеки оператор има асоциативност**. Това ще рече, когато имаме няколко оператора с еднакъв приоритет, действието се извършва или отляво-надясно (ляво асоциативни) или отдясно-наляво (дясно асоциативни).
...
тези оператори също връщат стойност, а именно стойността на х (или променливата, която променяме).
Какво ще изведе кода:
...
Нека усложним задачата. Нека сега се въвежда число, ако числото е между 2 и 6 да се извежда оценката с думи, която студента е получил. В противен случай да се изведе съобщение за грешка. Това можем да го постигнем със следната конструкция:
```cpp
if(<условие>)
{
    // направи нещо
